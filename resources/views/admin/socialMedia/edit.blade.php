    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.social-media.update", [$socialMedium->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('contact_card') ? 'is-invalid' : '' }}" name="contact_card_id" id="contact_card_id" required>
                    @foreach($contact_cards as $id => $entry)
                        <option value="{{ $id }}" {{ (old('contact_card_id') ? old('contact_card_id') : $socialMedium->contact_card->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('type') ? 'is-invalid' : '' }}" name="type" id="type" required>
                    ...
                    @foreach(App\Models\SocialMedium::TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('type', $socialMedium->type) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('type'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('link') ? 'is-invalid' : '' }}" type="text" name="link" id="link" value="{{ old('link', $socialMedium->link) }}" required>
                @if($errors->has('link'))
                    <div class="invalid-feedback">
                        {{ $errors->first('link') }}
                    </div>
